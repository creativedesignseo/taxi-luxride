      ...
      <div className="container mx-auto px-6 md:px-8 lg:px-16">
        <div className="flex flex-col lg:flex-row items-center gap-8 md:gap-12">
          ...
          <div className="flex-1 space-y-4 md:space-y-6 text-center lg:text-left">
            ...
            <h2 className="text-[32px] md:text-[48px] font-bold text-[#1C1F23] leading-tight font-['Inter',sans-serif]">
            ...
            <ul className="space-y-3 md:space-y-4 inline-block text-left">
              {['Confirmación inmediata', 'Sin cargos de reserva', 'Cancelación gratuita'].map((item, i) => (
                <li key={i} className="flex items-center gap-3">
                  <div className="w-5 h-5 bg-[#1C1F23] rounded-full flex items-center justify-center shrink-0">
                    <Check size={12} className="text-white" />
                  </div>
                  <span className="text-[#1C1F23] font-['Arimo',sans-serif]">{item}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex-1 w-full max-w-md">
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-lg border border-[rgba(3,2,19,0.05)]">
              <h3 className="text-xl font-bold text-[#1C1F23] mb-6 font-['Arimo',sans-serif]">Detalles de tu viaje</h3>
              <form className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[#1C1F23] mb-2 font-['Arimo',sans-serif]">Origen</label>
